...
public class FlexBase64Test {
    ...
    public void testEncoderOutputStream() throws Exception {
        ...
        stream.write(towel);
        stream.close();
        Assert.assertArrayEquals(TOWEL_BASE64.getBytes("US-ASCII"), baos.toByteArray());

        baos.reset();
        stream = FlexBase64.createEncoderOutputStream(baos, 8192, false);
        stream.write(towel);
        stream.close();
        Assert.assertArrayEquals(TOWEL_BASE64.replace("\r\n", "").getBytes("US-ASCII"), baos.toByteArray());
    ...
    public void testDecoderOutputStream() throws Exception {
        ...
        stream.write(towel);
        stream.close();
        Assert.assertArrayEquals(TOWEL.getBytes("US-ASCII"), baos.toByteArray());
